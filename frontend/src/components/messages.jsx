...
const Messages = () => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await axios.get("http://54.210.179.254:5000/api/messages");
  ...
  const handleSendReply = async () => {
    ...
    try {
      await axios.post(
        `http://54.210.179.254:5000/api/messages/${selectedMessage._id}/reply`,
        replyData
      );
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm("Are you sure you want to delete this message?")) {
      try {
        await axios.delete(`http://54.210.179.254:5000/api/messages/${id}`);
